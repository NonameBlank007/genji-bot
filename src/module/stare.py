...
@Help.register("stare", "stare.......", "Anime")
class StareModule(Module):
    @classmethod
    def setup(cls, app: Application):
        app.add_handler(CommandHandler("stare", st))


NEKOS_API = "https://nekos.best/api/v2/stare"
...
async def st(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    async with httpx.AsyncClient() as client:
        try:
            anime_name, gif = await fetch_st(client)
            reply_to = update.effective_message.reply_to_message.message_id if update.effective_message.reply_to_message else None
            logger.info(f"({update.message.from_user.id}): {update.effective_chat.title} ({update.message.chat_id}) used /stare")
            ...
            logger.critical("Failed to fetch stare")
            await update.message.reply_text("Sorry, I couldn't fetch a stare GIF right now. Please try again later")


async def fetch_st(client: httpx.AsyncClient):
