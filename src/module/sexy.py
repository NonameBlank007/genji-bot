...
@Help.register("sexy", "calculate sexyness for a day", "Fun")
class SexyModule(Module):
    @classmethod
    def setup(cls, app: Application):
        app.add_handler(CommandHandler("sexy", sexy_command))


SEXY_DATA_FILE = "sexy_data.json"


async def sexy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info(f"({update.message.from_user.id}): {update.effective_chat.title} ({update.message.chat_id}) used /sexy")
    await calc_sexy(update, context)


async def calc_sexy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    current_chat_id = str(chat_id)
    _private = update.effective_chat.type == "private"
    _entry = False
    now = datetime.now()
    user_data = load(SEXY_DATA_FILE)
    ...
    if _entry and not _private:
        await wrt(user_data_entry, user_id, user_data, SEXY_DATA_FILE)
    if ntd:
        calc = await update.message.reply_text(f"Calculating sexyness...", reply_to_message_id=reply_to)
        await asyncio.sleep(0.3)
        await calc.edit_text(f"{user_name} is {percentage}% sexy for today.")
    else:
        await update.message.reply_text(f"{user_name} is {percentage}% sexy for today.", reply_to_message_id=reply_to)
