...
NEKOS_API = "https://nekos.best/api/v2/hug"
...
async def hug(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    async with httpx.AsyncClient() as client:
        try:
            anime_name, gif = await fetch_hug(client)
            reply_to = update.effective_message.reply_to_message.message_id if update.effective_message.reply_to_message else None
            logger.info(f"({update.message.from_user.id}): {update.effective_chat.title} ({update.message.chat_id}) used /hug")
            ...
            logger.critical("Failed to fetch hug")
            await update.message.reply_text("Sorry, I couldn't fetch a hug GIF right now. Please try again later")


async def fetch_hug(client: httpx.AsyncClient):
