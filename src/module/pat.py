...
NEKOS_API = "https://nekos.best/api/v2/pat"
...
async def pat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    async with httpx.AsyncClient() as client:
        try:
            anime_name, gif = await fetch_pat(client)
            reply_to = update.effective_message.reply_to_message.message_id if update.effective_message.reply_to_message else None
            logger.info(f"({update.message.from_user.id}): {update.effective_chat.title} ({update.message.chat_id}) used /pat")
            ...
            logger.critical("Failed to fetch pat")
            await update.message.reply_text("Sorry, I couldn't fetch a pat GIF right now. Please try again later")


async def fetch_pat(client: httpx.AsyncClient):
