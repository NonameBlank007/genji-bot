...
class PokeModule(Module):
    ...
    def setup(cls, app: Application):
        app.add_handler(CommandHandler("poke", po))


NEKOS_API = "https://nekos.best/api/v2/poke"
...
async def po(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    async with httpx.AsyncClient() as client:
        try:
            anime_name, gif = await fetch_po(client)
            reply_to = update.effective_message.reply_to_message.message_id if update.effective_message.reply_to_message else None
            logger.info(f"({update.message.from_user.id}): {update.effective_chat.title} ({update.message.chat_id}) used /poke")
            ...
            logger.critical("Failed to fetch poke")
            await update.message.reply_text("Sorry, I couldn't fetch a poke GIF right now. Please try again later")


async def fetch_po(client: httpx.AsyncClient):
